        ...
        <div style={{ background: 'var(--surface)', borderRadius: compact ? '26px 26px 0 0' : 24, padding: compact ? '22px 22px 28px' : '26px 28px', width: '100%', maxWidth: compact ? '100%' : 520, boxShadow: 'var(--shadow-lg)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            <div style={{ lineHeight: 1.2 }}>
              <div style={{ fontSize: 16, fontWeight: 800 }}>민준이 · {f.key}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '20px 0 10px' }}>
            <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--text-sub)', whiteSpace: 'nowrap' }}>오늘 민준이 메모 한눈에</span>
    ...
    <PhoneFrame>
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '14px 20px 26px', background: 'linear-gradient(transparent, var(--bg) 28%)' }}>
          ...
          <button className="jr-btn jr-btn--primary jr-btn--block jr-btn--lg" disabled={empty}><Icon name="check" size={22} stroke={2.6} /> {empty ? '메모 저장' : '민준이 메모 저장'}</button>
        </div>
      </div>
      ...
      {saved && (
        <>
          ...
          <div style={{ position: 'absolute', left: 20, right: 20, bottom: 120, zIndex: 11, display: 'flex', justifyContent: 'center' }}>
            <div className="jr-toast" style={{ width: '100%', justifyContent: 'center' }}><span className="tick"><Icon name="check" size={14} stroke={3} /></span>민준이 페이지에 저장됐어요</div>
          </div>
        </>
      )}
    </PhoneFrame>
              ...
              <div>
                ...
                <div className="jr-card jr-card--soft" style={{ padding: 18, textAlign: 'center' }}>
                  ...
                  {empty
                    ...
                    : <p style={{ fontSize: 13, color: 'var(--text-faint)', textAlign: 'center', marginTop: 12, lineHeight: 1.5 }}>저장하면 <b style={{ color: 'var(--text-sub)' }}>민준이 페이지</b>에<br/>바로 기록돼요 · 영역은 밤에 자동 분류</p>}
                </div>
              </div>
            </div>
          </div>
