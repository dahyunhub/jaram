// ondo-ui.jsx — 온도 공유 UI 컴포넌트 (window 전역으로 내보냄)
...
// ---------- 데스크톱(브라우저) 윈도우 ----------
function DesktopWindow({ children, width = 1180, height = 760, url = 'app.ondo.kr' }) {
  return (
...
function Logo({ size = 32 }) {
  ...
  return (
    <img src="assets/logo-mark.png" alt="온도" width={size} height={size}
      style={{ flex: '0 0 auto', objectFit: 'contain', display: 'block' }} />
  );
}

// ---------- 로고 로더 (온도 마크가 위아래로 통통) ----------
...
function LogoLockup({ height = 44 }) {
  // 원본 858x439 → 비율 유지
  return (
    <img src="assets/logo-lockup.png" alt="온도" height={height}
...
function LogoVertical({ height = 120 }) {
  // 원본 411x666 → 비율 유지
  return (
    <img src="assets/logo-vertical.png" alt="온도" height={height}
      style={{ flex: '0 0 auto', width: 'auto', objectFit: 'contain', display: 'block' }} />
  );
}
...
function LogoTitle({ height = 40 }) {
  // 원본 584x292
  return (
    <img src="assets/logo-title.png" alt="온도" height={height}
      style={{ flex: '0 0 auto', width: 'auto', objectFit: 'contain', display: 'block' }} />
  );
}
