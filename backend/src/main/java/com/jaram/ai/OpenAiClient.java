...
import com.jaram.common.exception.AiAnalysisException;
...
public class OpenAiClient implements AiClient {
    ...
    public OpenAiClient(AiProperties props) {
        ...
        if (props.apiKey() == null || props.apiKey().isBlank()) {
            log.warn("AI api-key 미설정(jaram.ai.api-key / AI_API_KEY) — 실제 호출 시 401 로 실패한다");
        }
        if (this.model == null || this.model.isBlank()) {
            log.warn("AI model 미설정(jaram.ai.model / AI_MODEL) — 실제 호출 시 4xx 로 실패한다");
        }
    }
    ...
    private Map<String, Object> buildBody(AiRequest req) {
        ...
        if (req.responseSchema() != null) {
            body.put("response_format", Map.of(
                    "type", "json_schema",
                    "json_schema", Map.of("name", "jaram_result", "strict", true, "schema", req.responseSchema())));
        }
        return body;
    }
